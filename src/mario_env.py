...
SOLID_TILES = {1, 2, 3, 4, 7, 8, 9, 10, 11, 12, 13}  # Tiles Mario can stand on
...
class MarioGridEnv(gym.Env):
    ...
    def __init__(self, level_string: Optional[str] = None, tile_size: int = 16):
        ...
        # Action space: 0=noop, 1=right, 2=right+jump, 3=left, 4=left+jump, 5=jump
        self.action_space = spaces.Discrete(6)
        
        # Observation: local view around Mario (11x11 tiles) + velocity + state
        self.view_radius = 5
...
""".strip()
    ...
    def load_level(self, level_string: str):
        ...
        # Find spawn on ground if not specified
    ...
    def reset(self) -> np.ndarray:
        ...
        # Episode tracking (for HMM metrics)
        self.lives = 3
        self.deaths = 0
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        if self.mario_y >= self.level_height:  # Fell off
            self.done = True
            self.deaths += 1
            reward -= 50
        elif self.steps >= 2000:  # Timeout
            self.done = True
    ...
    def _apply_action(self, action: int):
        ...
        if action in [1, 2]:  # Right
            self.mario_vx = self.move_speed
        elif action in [3, 4]:  # Left
            self.mario_vx = -self.move_speed
        else:
            self.mario_vx *= 0.8
        
        if action in [2, 4, 5] and self.on_ground:  # Jump
    ...
    def _hit_block(self, x: int, y: int):
        ...
        if 0 <= y < self.level_height and 0 <= x < self.level_width:
            ...
            elif tile == 2 and self.has_mushroom:  # Breakable
    ...
    def _check_collisions(self):
        ...
        for enemy in self.enemies[:]:
            ...
            if dx < 0.8 and abs(dy) < 1.0:
                if self.mario_vy > 0 and dy < -0.3:  # Stomping
    ...
    def _calculate_reward(self, prev_x: float) -> float:
        ...
        reward = (self.mario_x - prev_x) * 1.0  # Progress reward
