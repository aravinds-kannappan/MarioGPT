...
class DatasetConfig:
    ...
    frame_width: int = 64
    frame_height: int = 64
    frame_skip: int = 4  # Record every Nth frame (like GameNGen)
...
# DeepRTS Action Space (15 discrete actions)
DEEPRTS_ACTIONS = {
    0: "IDLE",
    1: "MOVE_UP",
    2: "MOVE_UP_RIGHT", 
...
class DeepRTSEnvWrapper:
    ...
    def _setup_env(self):
        ...
        try:
            ...
            # Create environment using scenario
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        # In DeepRTS, need to set active player
...
class MockDeepRTSEnv:
    ...
    def _generate_mock_state(self):
        ...
        frame = np.zeros((self.config.frame_height, self.config.frame_width, 3), 
                        dtype=np.uint8)
        # Add some mock game elements
        # Green for terrain
        frame[:, :, 1] = np.random.randint(20, 50, frame.shape[:2], dtype=np.uint8)
        # Add mock units (small colored squares)
...
class HeuristicAgent:
    ...
    def get_action(self, state: np.ndarray) -> int:
        # 70% chance to continue current action (smoother gameplay)
        if self.action_repeat_count < 5 and random.random() < 0.7:
            self.action_repeat_count += 1
            return self.last_action
        
        # Weight actions by strategic importance
        weights = [
            0.05,  # IDLE
...
def collect_episode(
    ...
    while not done and step < config.max_steps_per_episode:
        ...
        # Execute frame_skip steps with same action
...
def save_episode(
    ...
    # Save actions (GameNGen style - one per line)
