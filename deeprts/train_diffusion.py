...
class TrainingConfig:
    ...
    image_size: int = 64
    buffer_size: int = 8  # Number of past frames to condition on
    ...
    zero_out_action_prob: float = 0.1  # For classifier-free guidance
    
    # Noise augmentation (from GameNGen paper)
...
class DeepRTSDataset(Dataset):
    ...
    def _index_episodes(self, max_samples: Optional[int]):
        ...
        for episode_dir in episode_dirs:
            ...
            # Create samples starting from buffer_size (need context)
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # Load context frames (buffer_size frames before target)
        context_frames = []
        context_actions = []
...
class ContextEncoder(nn.Module):
    ...
    def __init__(
        ...
        buffer_size: int = 8,
        hidden_dim: int = 256,
        out_dim: int = 768  # Match CLIP embedding dim for SD compatibility
    ):
        super().__init__()
        self.buffer_size = buffer_size
    ...
    def forward(self, frames: torch.Tensor) -> torch.Tensor:
        ...
        # Encode each frame
        frames_flat = frames.view(-1, *frames.shape[2:])  # (batch*buffer, C, H, W)
        frame_embeds = self.frame_encoder(frames_flat)  # (batch*buffer, hidden)
        ...
        out = self.proj(pooled)  # (batch, out_dim)
        
        return out.unsqueeze(1)  # (batch, 1, out_dim) for cross-attention
...
class DeepRTSTrainer:
    ...
    def _init_models(self):
        ...
        if DIFFUSERS_AVAILABLE and self.config.use_pretrained:
            try:
                # Use pretrained VAE and UNet from Stable Diffusion
                ...
                # Custom UNet that accepts our conditioning
    ...
    def add_noise_augmentation(
        ...
        # Sample noise bucket for each frame
    ...
    def training_step(self, batch: dict) -> torch.Tensor:
        ...
        # Encode target to latent space if using VAE
        if self.use_vae:
            with torch.no_grad():
